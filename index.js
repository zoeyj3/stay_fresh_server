...
app.get("/inventory", (req, res) => {
  ...
  parsedInventoryList = parsedInventoryList.map((item) =>
    addDaysToExpire(item)
  );
...
app.get("/inventory/:id", (req, res) => {
  ...
  const individualInventory = parsedInventoryList.filter(
    (item) => item.id == req.params.id
  );
  // id validation check
  const inventoryChecked = parsedInventoryList.find(
    (item) => item.id == req.params.id
  );
  if (!inventoryChecked) {
...
app.get("/inventory-name/:name", (req, res) => {
  const parsedInventoryList = readFullInventoryList();
  let individualInventory = parsedInventoryList.filter((item) =>
    item.name.toUpperCase().includes(req.params.name.toUpperCase())
  );
  // name validation check
  if (!individualInventory || individualInventory.length === 0) {
    return res.status(404).send("item not found");
  }
  individualInventory = individualInventory.map((item) =>
    addDaysToExpire(item)
  );
  res.json(individualInventory);
...
app.post("/add", (req, res) => {
  const parsedInventoryList = readFullInventoryList();
  const inventoryRepeatList = parsedInventoryList.find(
    (item) => item.name.toUpperCase() == req.body.name.toUpperCase()
  );
...
app.put("/inventory/:id", (req, res) => {
  ...
  fs.writeFileSync(
    "./data/inventory.json",
    JSON.stringify(parsedInventoryList)
  );
